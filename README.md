...
SELECT DISTINCT
	...
	,ICD.[SECONDARY ICD10 CODE] AS [Secondary Diagnosis Code]
	...
	,ISNULL(CDF.PA_EXP_DATE,'')[Primary PA Expiration Date]
	...
	,ISNULL(BST.LOG_DTTM,'') AS [BI Start Date]
	,ISNULL(CDF.PA_START_DATE,'') AS [PA Start Date]
	,ISNULL(REPLACE(PAR.PA_RCVD_DTTM,'-',''),'') AS [PA Approved Date]
	,REPLACE(ISNULL(AD.PA_DENIED_DTTM,''),'-','') AS [PA Denied Date]
	,REPLACE(ISNULL(ASD.APPEAL_START_DATE,''),'-','') AS [First Appeal Start Date]
